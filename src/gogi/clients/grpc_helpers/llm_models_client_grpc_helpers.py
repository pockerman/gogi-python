...
from gogi.models.llm.llm_tool_definition import (
    LLMToolCall,
    LLMToolDefinition,
    ToolCallFunction,
)
...
from gogi.models.llm.requests.list_registered_llm_request import (
...
from gogi.models.llm.responses.list_registered_llms_response import (
...
class LLMModelsClientGRPCHelper:
    ...
    @staticmethod
    def validate_provider_supports_model(request: LLMRunRequest, models: list[str] | None) -> None:
        if models is None:
    ...
    def request_config_to_grpc_request_config(config: LLMRunRequestConfig) -> llm_model_service_pb2.LLMRunRequestConfig:
        return llm_model_service_pb2.LLMRunRequestConfig(
            model=config.model,
    ...
    def build_grpc_request(req: LLMRunRequest) -> llm_model_service_pb2.LLMRunRequest:
        ...
        response_format = (
            llm_model_service_pb2.ResponseFormat(
                type=req.response_format.response_type, schema_json=req.response_format.response_schema_json
            )
            if req.response_format
            else None
        )

        return llm_model_service_pb2.LLMRunRequest(
            messages=messages, config=config, tools=tools, response_format=response_format
        )

    @staticmethod
    def build_grpc_capabilities(request: LLMRegisterRequest) -> llm_model_service_pb2.LLMCapabilities:
        return llm_model_service_pb2.LLMCapabilities(
            context_window=request.info.capabilities.context_window,
    ...
    def build_grpc_registration_request(request: LLMRegisterRequest) -> llm_model_service_pb2.RegisterLLMRequest:
        return llm_model_service_pb2.RegisterLLMRequest(
            info=llm_model_service_pb2.ModelInfo(
    ...
    def grpc_token_usage_to_token_usage(grpc_usage: Any | None) -> LLMTokenUsage:
        ...
        return LLMTokenUsage(
            prompt_tokens=grpc_usage.prompt_tokens,
    ...
    def grpc_tool_calls_to_tool_call(grpc_tool_calls: list[Any]) -> list[LLMToolCall]:
        ...
        return [
            LLMToolCall(
                idx=tool.id,
                tool_type=tool.type,
                function=ToolCallFunction(name=tool.function.name, arguments=tool.function.arguments),
            )
            for tool in grpc_tool_calls
        ]

    @staticmethod
    def list_registered_llms_request_to_grpc(
        request: ListRegisteredLLMsRequest,
    ) -> llm_model_service_pb2.ListRegisteredLLMsRequest:
    ...
    @staticmethod
    def serialze_get_llm_status_grpc_response(
        grpc_response: llm_model_service_pb2.LLMStatusResponse,
    ) -> LLMStatusResponse:
        return LLMStatusResponse(
            name=grpc_response.name,
            status=grpc_response.status,
            endpoint=grpc_response.endpoint,
            last_checked=grpc_response.last_checked,
        )

    @staticmethod
    def get_llm_capabilities_to_grpc(
        request: GetLLMCapabilitiesRequest,
    ) -> llm_model_service_pb2.GetLLMCapabilitiesRequest:
        return llm_model_service_pb2.GetLLMCapabilitiesRequest(model=request.model)

    @staticmethod
    def serialze_get_llm_capabilities_grpc_response(
        grpc_response: llm_model_service_pb2.LLMCapabilitiesResponse,
    ) -> LLMCapabilitiesResponse:
        return LLMCapabilitiesResponse(
            capabilities=LLMModelsClientGRPCHelper.serialize_grpc_llm_capabilities(grpc_response.capabilities)
        )
