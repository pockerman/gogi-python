...
class LLMModelsClient(BaseClient):
    def __init__(self, platform, logger=None):
        ...
        self._providers_to_model_cache: dict[str, list[str]] | None = None
    ...
    def run_stream(self, request: LLMRunRequest) -> Iterator[LLMRunStreamChunk]:
        ...
        try:
            ...
            if self.logger:
                self.logger.warning("run_stream failed: %s", e)
            raise
    ...
    def get_llm_providers(self) -> list[LLMProvider]:
        response = self._stub.GetLLMProviders(
            llm_model_service_pb2.GetLLMProvidersRequest(fetch_models=True), metadata=self.route_metadata
        )
    ...
    def register_llm(self, request: LLMRegisterRequest) -> LLMRegisterResponse:
        ...
        return LLMRegisterResponse(
            name=grpc_response.name, status=grpc_response.status, registered_at=grpc_response.registered_at
        )
